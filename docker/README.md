...
либо в папке с уже имеющимся локальным гит репозиторием создаём remote url который соответствует файловому пути к папке моего проекта в гитрепозитории (Внимание - откорректируйте комманду чтобы она включала полный путь к репозиторию на гитсервере)
...
если наоборот, в старой песочнице возникнет необходимость подключить новые агенты из текущей песочницы, то для этого при одновременно работающем старом и новом окружении нужно в файле docker/exec_ssh-keyscan.py в двух местах раскомментировать 
```
jenkins_builtin_container_name = "jenkins_sandbox"
# jenkins_builtin_container_name = "jenkins-blueocean"
```

сохранить и запустить exec_ssh-keyscan.py на исполнение. При этом в старый built-in jenkins контейнер будут прописаны хосты новых нодов и гитсервера. Но предварительно в докерфайл built-in jenkins старого окружения должна быть вставлена инструкция
...
а также в папку с этим докерфайлом должен быть скопирован файл scan-host-key.sh
